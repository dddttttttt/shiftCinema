...
const FilmInformation = ({inList, film}) => {
    ...
    for (let i = 1; i <= 5; i++) {
        if (i*2 <= rating)
            starsRating.push(<img src="/shiftCinema/Icons/starFilledIcon.svg" key={i}/>);
        else if (i*2-1 <= rating)
            starsRating.push(<img src="/shiftCinema/Icons/starHalfIcon.svg" key={i}/>);
        else
            starsRating.push(<img src="/shiftCinema/Icons/starEmptyIcon.svg" key={i}/>);
    }
